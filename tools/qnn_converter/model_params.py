...
# Qwen3-4B
class Qwen3_4B_Params(ModelParams):
    has_qkv_bias = False
    use_drelu = False
    tie_embedding = True
    ...
    vocab_size = 151936    
    ffn_hidden_dim = 9728  # intermediate_size
    head_dim = 128         # head_dim
    n_heads = 32           # num_attention_heads
    n_kv_heads = 8         # num_key_value_heads
    embed_dim = 2560       # hidden_size

    # RoPE 和 Norm 参数
    # 根据 JSON: "rope_theta": 1000000, "rms_norm_eps": 1e-06
    rope_theta = 1e6
    rms_norm_eps = 1e-6
    attention_mask_value = -5e4

    # FP16 策略配置 (参考 0.6B 设置)
    fp16_attention_layers = []
    fp16_ffn_layers = []
    fp16_rope = True
    fp16_qkv_heads =  True
